...
subroutine compute_transition(tau_h_path,KN_path,hsv_0_path,tau_p_path,pol_path_out,agg_path_out,val_t1,val_final, &
	...
	real(8),allocatable :: share_entre_path(:,:),inc_share_EP_path(:), inc_share_ES_path(:), inc_share_EC_path(:), share_wage_ES_path(:),netinc_share_EP_path(:), netinc_share_ES_path(:), netinc_share_EC_path(:),trans_mat_path(:,:,:)
	...
	allocate(KN_path_old(T),hsv_0_path_old(T),tau_p_path_old(T),pol_path(T),T_vec(T),agg_path(T), &
		prices_path(T),avek_path(T,4),share_entre_path(T,4),inc_share_EP_path(T), inc_share_ES_path(T), inc_share_EC_path(T), share_wage_ES_path(T),netinc_share_EP_path(T), netinc_share_ES_path(T), netinc_share_EC_path(T),trans_mat_path(T,4,4))	
...
! 			write(*,*) "pen",prices_t%pen
		...
		do t_c = 1,T-1
			...
			trans_mat_path(t_c,:,:) = model_targets_t%trans_mat
		enddo ! t_c		
	...
	subroutine sub_write_transition(dir)
		...
		call writedim(trans_mat_path, trim(dir)//trim('trans_mat_path.txt'))
