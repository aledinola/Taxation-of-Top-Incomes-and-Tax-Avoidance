...
do ith = 1,ntheta ! Current theta shock
    ...
    do ia = 1,na ! Current assets
        ...
        if (cf_avoidance == 0 .or. cf_avoidance == 4) then
        	! baseline model environment or counterfactual: no C-corp
	...
	do ie = 1,neps  ! current state - epsilon
		do ia = 1,na
			...
			if (cf_avoidance == 0) then
    			...
    			enddo 
    		elseif (cf_avoidance == 1 .or. cf_avoidance == 2 .or. cf_avoidance == 3 .or. cf_avoidance == 4) then
    			! Counterfactual: no tax avoidance on the intensive or extensive margins
				if (cf_occpol == 0 .and. (cf_avoidance == 1 .or. cf_avoidance == 2)) then
    				V_aux(3) = large_negative
				endif
				! Counterfactual: all entrepreneurs are sole-prop. (EP)
				if (cf_avoidance == 3) then
					V_aux(3) = large_negative
					V_aux(4) = large_negative
				endif
				! Counterfactual: no C-corp (all entre have the same lambda)
				if (cf_avoidance == 4) then
					V_aux(4) = large_negative
				endif
    			do iz_min = 1,nz
					call sub_logsum(newVy(ia,ie,it,iz_min),prob_aux, V_aux-switch_cost(iz_min,:),sig_e) 
