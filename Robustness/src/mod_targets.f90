...
contains
    ...
    real(8) :: t1,t2
    real(8) :: theta_val,eps_val,a_val,l_val,k_val,nbar_val,phi_val,profit_val,n_val,P_occ,yw
    ...
    ! * Definition for taxableinc: (gross) income subject to income taxation
...
! 	write(*,*) "Calculate shares of work/sole-prop/S-corp/C-corp/Retirees...done!"
    ...
    ! Total profits of sole-prop
    !-------------------------------------------------------------
    call get_profits(profits,pol,mu)    
    ...
    ! - Total business income of sole-prop (relevant for Exp 5)
	model_targets%profits = profits
...
!     call CPU_TIME(t2)
    ...
    subroutine get_profits(profits,pol,mu)
    implicit none
    ! Purpose: compute total profits of sole-proprietorship
    ! Inputs:  none
    real(8), intent(in) :: mu(:,:,:,:)
    type(polfun), intent(in) :: pol
    ! Outputs: profits
    real(8), intent(out) :: profits
    ! Locals
    integer :: it,ie,ia,io
    real(8) :: theta_val,eps_val,a_val,k_val,n_val,profit_val

    ! Compute total profits of sole-prop 
    profits = 0.0d0
    do it = 1,ntheta
        theta_val  = theta_grid(it)           
        do ie = 1,neps
	        ...
	        do ia = 1,na
                ...
                io=2 ! Sole Proprietor
                k_val = pol%kpol(ia,it,io)
                n_val = pol%npol(ia,it,io)
                profit_val = fun_profit(theta_val,k_val,le,n_val,r,w) 
                profits = profits + profit_val*mu(ia,ie,it,io)
                
                io = 3 ! S-Corps
                k_val = pol%kpol(ia,it,io)
                n_val = pol%npol(ia,it,io)
                phi_val  = pol%phipol(ia,it,io)
                profit_val = phi_val*fun_profit(theta_val,k_val,le,n_val,r,w) 
                profits = profits + profit_val*mu(ia,ie,it,io)
...
!===============================================================================!
    ...
    ! Outputs: atr_vec(up to P50, P50-P90, P90-P99, above P99)
    ...
    real(8) :: qw(4), inc_p(0:4), tot_taxableinc(4),tot_inctax(4),temp_vec(4)
	integer :: istat,iq
	...
	! All percentiles we want to look at
    qw = [0.99d0,0.90d0,0.5d0,0.0d0] 
	! Income percentiles corresponding to qw
    !write(*,*) "atr_by_inc: now calling function quantili"
    inc_p(1:4) = quantili(taxableinc_vec,mu_vec,qw)
    inc_p(0)   = huge(0.0d0)
    inc_p(4)   = -huge(0.0d0)
...
!===============================================================================!
	...
	write(uno, '(a)')' '
    write(uno, '(a)')'ATR'
	write(uno, '(a,f12.6)')'Top 1% : ', model_targets%atr_vec(1)
	write(uno, '(a,f12.6)')'P90-P99 : ', model_targets%atr_vec(2)
	write(uno, '(a,f12.6)')'P50-P90 : ', model_targets%atr_vec(3)
	write(uno, '(a,f12.6)')'Bottom 50% : ', model_targets%atr_vec(4)
    ...
    write(uno, '(a)')' '
    write(uno, '(a,f12.6)')'profits : ', model_targets%profits
    close(uno)
