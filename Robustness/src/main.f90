...
! SET SOME FLAGS HERE 
!============================================================================
cf_avoidance   = 3		! 0 = baseline environment with avoidance
...
do_fix_pen     = 1		! 0 = pension replacement rate is fixed, but pension level is endogeneous, 1 = pension level is fixed (so that replacement rate is endogeneous). Only relevant if do_benchmark = .false.
...
mydebug        = 0
do_write       = 0       ! 1= write results of comparative statics to txt
...
if (do_run == 0) then
	...
	if (do_benchmark) then
		...
		if ((do_GE == 2 .or. do_GE==3) .and. (cf_avoidance == 1 .or. cf_avoidance ==2 .or. cf_avoidance == 3)) then ! Fiscal neutrality tax avoidance
			...
			elseif (do_GE == 2 .and. cf_avoidance==3 .and. cf_occpol == 1) then
				! Experiment 6: no S- or C-corporations, fixed occpol
...
elseif (do_run==5) then
	...
	! Read occupational choice from the benchmark economy (occpol_bench is a global object)
	if (allocated(occpol_bench)) deallocate(occpol_bench)
	allocate(occpol_bench(na,neps,ntheta,nz,no), stat=istat)
	if (istat/=0) then
		call myerror("occpol_bench: Allocation failed!")
	endif
